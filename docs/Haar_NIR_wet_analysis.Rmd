...
ggsave("wetNIR_prop_hist_20260313.png", width = 9, height = 4, 
...
ggplot(data = NIR_hist.df %>%
  ...
  geom_histogram(aes(x = value)) +
  facet_wrap(~ name, scales = "free_x") +
  labs(x = "Property Value", y = "Distribution", title = "KSSL Samples") +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),
        plot.title = element_text(size = 13, color = "Black", hjust = 0.5),
        axis.title = element_text(size = 13, color = "Black"),
...
ggsave("NIR_prop_hist_20260313.png", width = 9, height = 4, 
       path = fig_dir, dpi = 300, units = "in")

```

Combined wet and KSSL organic carbon, pH, and clay distributions.

```{r KSSL+wet_lab_vis}

# combining KSSL and wet sample lab measured organic carbon, pH, and clay
# ...distributions
ggplot(data = NIR_wet_hist.df %>%
         mutate(data = "Soil Wetness Samples") %>%
         relocate('Clay (%)', .after = sample_id) %>%
         relocate('pH-H2O', .after = 'OC (%)') %>%
         rbind(NIR_hist.df %>%
                 mutate(data = "KSSL Samples")) %>%
         pivot_longer(cols = -c(sample_id, data))) +
  geom_histogram(aes(x = value)) +
...
# Saving combined soil property distributions
ggsave("Combined_KSSLwet_hists_20260313.png", width = 8, height = 6, 
       path = fig_dir, dpi = 300, units = "in")
...
# Break up Haar NIR to get separated numeric Haar order and level columns, 
# ...with signal order preserving the order for plotting - and select sample
longHaar <- Haar_NIR %>%
  filter(sample_id == "36035_3") %>%
  pivot_longer(-sample_id, 
               names_to = 'Haar_id', values_to = 'values') %>%
  separate_wider_delim('Haar_id', delim = '_', 
                       names = c('Haar_level', 'Haar_order'), 
                       cols_remove = FALSE) %>%
  mutate(Haar_level = as.numeric(str_remove(Haar_level, '^H')),
         Haar_order = as.numeric(str_remove(Haar_order, '^I'))) %>%
  arrange(Haar_level, Haar_order) %>%
  mutate(signal_order = 1:n()) %>%
  mutate(signal_order_rev = rev(signal_order), .by = Haar_level)

# Selecting the last Haar signal in levels 9 through 5 and 1, to place x-axis 
# ...breaks in figures below
HaarBreaks <- longHaar %>%
  filter(Haar_order == 1,
         Haar_level %in% c(9:5, 1))

# Plotting Haar signal through transform levels
H_NIR_scan_plot <- ggplot(longHaar, aes(x = signal_order_rev, y = values)) +
  geom_line() +
  labs(y = "Transformed Reflectance", x = "Haar Transform Levels") +
  scale_x_continuous(breaks = HaarBreaks$signal_order_rev,
                     #labels = HaarBreaks$Haar_level) +
                     labels = 10-HaarBreaks$Haar_level) +
  theme_bw() +
  theme(legend.position = "none",
        panel.background = element_blank(),
        panel.grid.major = element_blank(),
...
# Saving NIR scan plot
ggsave("Haar_NIR_plot_20260313.png", width = 6, height = 4,
       path = fig_dir, dpi = 300, units = "in")
...
All_BT_scan_plot <- ggplot(data = full_back_trans.df %>%
                             filter(MRA_value == 0 |
                                    ...
                                    MRA_value == 7 |
                                    MRA_value == 8) %>%
                             mutate(MRA_value = gsub(0, "Full Haar", MRA_value),
                                    MRA_value = gsub(2, "50% Haar", MRA_value),
                                    MRA_value = gsub(4, "12% Haar", MRA_value),
                                    MRA_value = gsub(6, "3.0% Haar", MRA_value),
                                    MRA_value = gsub(7, "1.0% Haar", MRA_value),
                                    MRA_value = gsub(8, "0.5% Haar", 
                                                     MRA_value)),
                           aes(x = Wavelength, y = Reflectance, 
                                 color = as.factor(MRA_value))) +
  geom_line() +
  geom_point(data = full_back_trans.df %>% 
               mutate(MRA_value = gsub(8, "1.0% Haar", MRA_value)) %>%
               filter(MRA_value == "1.0% Haar"), 
             aes(x = Wavelength, y = Reflectance, 
                 color = as.factor(MRA_value))) +
  labs(y = "Back-transformed Reflectance", x = "Wavelength (nm)") +
  theme_bw() +
  theme(legend.position = "bottom",
        ...
        axis.title = element_text(size = 13, color = "Black"),
        axis.text = element_text(size = 13, color = "Black")) +
  scale_color_grey(limits = c("Full Haar", "50% Haar", "12% Haar", "3.0% Haar",
                              "1.0% Haar", "0.5% Haar")) +
  #scale_x_reverse() +
...
# Saving reverse Haar NIR scan plot
ggsave("All_levels_RevHaar_NIR_plot_20260310.png", width = 8, height = 4,
       path = fig_dir, dpi = 300, units = "in")
...
ggsave("NIRHaarRHaar_plots_20260313.png", width = 9, height = 9,
       path = fig_dir, dpi = 300, units = "in")
