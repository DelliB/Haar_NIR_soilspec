...
ggsave("NIR_prop_hist_20251118.png", width = 9, height = 4, 
...
ggsave("NIR_scan_plot_20260109.png", width = 6, height = 4,
       path = fig_dir, dpi = 300, units = "in")
...
if(!file.exists(file.path(NIR_dir, 
                          'Haar_NIR_train_data_20251117.csv'))){
  ...
  write_csv(transformed_NIR, file.path(NIR_dir, "Haar_NIR_train_data_20251117.csv"))
...
H_NIR_scan_plot <- ggplot(data = Haar_NIR %>%
         ...
         mutate(Haar_level = sub(pattern = 'H', replacement = '', x = Haar_level)) %>%
         mutate(Haar_level = sub(pattern = 'I', replacement = '', x = Haar_level)) %>%
         mutate(Haar_level = sub(pattern = '_', replacement = '.', x = Haar_level)) %>%
         mutate(Haar_level = as.numeric(Haar_level)),
       aes(x = Haar_level, y = Value, group=1)) +
  geom_line() +
  xlim(0.0, 12.3) +
  labs(y = "Energy", x = "Haar level") +
  theme_bw() +
  theme(legend.position = "none",
        panel.background = element_blank(),
        panel.grid.major = element_blank(),
...
# Saving NIR scan plot
ggsave("Haar_NIR_plot_20260109.png", width = 6, height = 4,
       path = fig_dir, dpi = 300, units = "in")
...
ggsave("RevHaar_NIR_plot_20260109.png", width = 6, height = 4,
       path = fig_dir, dpi = 300, units = "in")

# Organizing full_back_trans.df to enable proper labeling of Haar levels for 
# ...back transform plot
full_back_trans_labels.df <- full_back_trans.df %>%
  #mutate(Reflectance = rev(Reflectance), .by = MRA_value) %>%
  mutate(signal_def = ifelse(MRA_value == '0', 
                             'Full Haar', MRA_value),
         signal_def = ifelse(MRA_value == '1', 
                             '60% Haar', signal_def),
         signal_def = ifelse(MRA_value == '2', 
                             '30% Haar', signal_def),
         signal_def = ifelse(MRA_value == '3', 
                             '15% Haar', signal_def),
         signal_def = ifelse(MRA_value == '4', 
                             '8.0% Haar', signal_def),
         signal_def = ifelse(MRA_value == '5', 
                             '4.0% Haar', signal_def),
         signal_def = ifelse(MRA_value == '6', 
                             '2.0% Haar', signal_def),
         signal_def = ifelse(MRA_value == '7', 
                             '1.0% Haar', signal_def),
         signal_def = ifelse(MRA_value == '8', 
                             '0.5% Haar', signal_def)) %>%
  mutate(line_type = ifelse(MRA_value == 6, "solid", "longdashed"))

# Full and compressed Haar signal back-transformed NIR scan plot
All_BT_scan_plot <- ggplot(data = full_back_trans_labels.df,
                           aes(x = Wavelength, y = Reflectance, 
                                 color = as.factor(signal_def),
                               linetype = line_type)) +
  geom_line() +
  geom_point(data = full_back_trans_labels.df %>% 
               filter(MRA_value == 8), 
             aes(x = Wavelength, y = Reflectance, 
                 color = as.factor(signal_def))) +
  labs(y = "Reflectance", x = "Wavelength (nm)") +
  ...
  theme(legend.position = "bottom",
        ...
        axis.title = element_text(size = 13, color = "Black"),
        axis.text = element_text(size = 13, color = "Black")) +
  scale_color_grey(limits = c("Full Haar", "60% Haar", "30% Haar", "15% Haar",
                              "8.0% Haar", "4.0% Haar", "2.0% Haar", 
                              "1.0% Haar", "0.5% Haar")) +
  #scale_x_reverse() +
  guides(linetype = "none", color = 
           guide_legend(nrow = 2, override.aes = list(shape = NA)))

print(All_BT_scan_plot)

# Saving reverse Haar NIR scan plot
ggsave("All_levels_RevHaar_NIR_plot_20260109.png", width = 8, height = 4,
       path = fig_dir, dpi = 300, units = "in")
...
ggsave("NIRHaarRHaar_plots_EDITS_20260109.png", width = 9, height = 9,
       path = fig_dir, dpi = 300, units = "in")
...
ggsave("wetNIR_prop_hist_20251118.png", width = 9, height = 4, 
       path = fig_dir, dpi = 300, units = "in")
